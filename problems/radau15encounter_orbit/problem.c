...
void problem_output(){
	struct particle com = particles[1];
	com =tools_get_center_of_mass(particles[1],particles[2]);
		
	FILE* of = fopen("position1.txt","a+"); 
	fprintf(of,"%f\t%f\t",particles[1].x-com.x,particles[1].y-com.y);
	fprintf(of,"%f\t%f",particles[2].x-com.x,particles[2].y-com.y);
	fprintf(of,"\n");
	fclose(of);
}
