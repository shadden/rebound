...
int integrator_ias15_step() {
	...
	int iterations = 2;
	if (t==0) iterations = 6;
	while(iterations){						// Predictor corrector loop
		iterations--;
				printf("here\n");
	...
	const double safety_factor = 0.75;  
	double H_max = 0;
	for(int k=0;k<N3;k++) {  // Looping over all particles and all 3 components of the acceleration. 
		double b6k = fabs(b[6][k]); 
		double H = b6k/(72.*pow(dt_done,7.));
		if (H>H_max){
			H_max= H;
		}
	}

	double dt_new = pow(integrator_epsilon/H_max,1./9.)*safety_factor; 
	dt = dt_new;
