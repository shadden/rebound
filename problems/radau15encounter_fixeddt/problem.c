...
void problem_finish(){
	...
	t= tmax;
	problem_output();	
	of = fopen("lastfrac.txt","a+"); 
	fprintf(of,"\n");
	fclose(of);
}
double outputs_max = 512;
double outputs = 0;
void problem_output(){
	while(t/tmax>outputs/outputs_max){
		FILE* of = fopen("lastfrac.txt","a+"); 
		fprintf(of,"%e %e %e\n",outputs/outputs_max*tmax/2./M_PI,log10(dt/2./M_PI),integrator_error);
		fclose(of);
		outputs++;
	}
}
